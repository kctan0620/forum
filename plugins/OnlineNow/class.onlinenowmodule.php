         ...
         if ($this->_OnlineUsers->NumRows() > 0) { 
                foreach($this->_OnlineUsers->Result() as $User) {
                    ...
                    if (($Session->CheckPermission('Plugins.OnlineNow.ViewHidden')) || ($User->Invisible < 1)) {
                       ...
                       $Href = Url(UserUrl($User));
                       echo '<div class="Time">'.'<a class="'.'Popup Name'.'" title="'.'Post to Activity'.'" href="'.$Href.'"'.'>'.htmlspecialchars($User->Name).'</a>'.'<a class="'.'Popup Time'.'" href="'.'messages/add/'.htmlspecialchars($User->Name).'" title="'.'Send PM'.'"'.'>'.Gdn_Format::Date($User->Timestamp).'</a>'.'</div>';
